...
function ObjGasto() {
    ...
    async function confirmarModal() {
        ...
        try {
            ...
            cerrarModal()
            await cargarLista()
            setStatusMsg('✔ Operación exitosa.', 'ok')
        } catch (e) {
    ...
    function accion(tipo) {
        if (tipo === 'nuevo')    { abrirModal('nuevo') }
        if (tipo === 'editar')   { if (!selected) { setStatusMsg('Seleccione una fila para editar.', 'error'); return } abrirModal('editar', selected) }
        if (tipo === 'eliminar') { eliminar() }
        if (tipo === 'salir')    { window.history.back() }
    }
    ...
    return (
        <>
            <section className="encabezado-marca">
                <article className="contenido-marca">
                    <section className="texto-marca">
            ...
            <main className="window-frame">
                <div className="window-titlebar">&#9632; Objgasto</div>

                <section className="window-body">
                    <h2 className="section-title">GESTIÓN DE OBJETO DE GASTO</h2>
                    ...
                    <div className="panel">
                        <div className="table-wrapper">
                            ...
                            <aside className="vscrollbar" aria-hidden="true">
                    ...
                    <nav className="button-bar" aria-label="Acciones">
                        <button className="btn" onClick={() => accion('nuevo')}>Nuevo</button>
                        <button className="btn" onClick={() => accion('editar')}>Editar</button>
                        <button className="btn" onClick={() => accion('eliminar')}>Eliminar</button>
                        <button className="btn" onClick={() => accion('salir')}>Salir</button>
                    </nav>
                </section>

                <footer className={`status-bar ${status.tipo}`} role="status" aria-live="polite">
            ...
            {showModal && (
                <dialog className="modal-window" open aria-labelledby="modalTitle">
                    <header className="modal-titlebar">
                        <span id="modalTitle">{modalMode === 'nuevo' ? 'Nuevo Objeto de Gasto' : 'Editar Objeto de Gasto'}</span>
                        <button className="close-btn" onClick={cerrarModal} aria-label="Cerrar">✕</button>
                    </header>
